...
## 📝메모
**내림차순 정렬 차이점** <br><br>
1️⃣ `num.sort((a, b) => b - a);`
- 배열을 **숫자 순**으로 정렬. 

2️⃣ `num.sort().reverse();`
- `reverse`는 배열을 기본적으로 **문자열로 정렬**하고 그 후에 배열을 뒤집는다.<br>
  문자열로 된 숫자를 정렬하므로 숫자의 실제 순서가 아닌 문자열로 된 숫자의 순서로 정렬됨!<br>
  결과적으로 10 이상의 숫자에서는 제대로 정렬이 이루어지지 않음 ('5' > '10'으로 인식)
